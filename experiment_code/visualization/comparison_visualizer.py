...
class FormatHandler:
    ...
    def calculate_balanced_scores(self, 
        ...
        """
        # Constant definitions to improve readability and facilitate adjustment
        ...
        # Get current model configuration, use default configuration if no specific configuration exists
        ...
        # Citation quality: improve citation score based on citation count
        ...
        if actual_answer_count == 1 and safe_expected_count > 1:
            ...
            # Adjust missing penalty based on answer quality: high-quality answers have lighter missing penalties
    ...
    #                            model: str) -> Tuple[float, float, float]:
    #     """
    #     Calculate balanced scores using the same method as EnhancedVisualizer
    ...
    #     # For citation score, boost if citation count is good
    ...
    #         if expected_answer_count+answer_count_difference==1:
    #             penalty=0.3
    #             quantity_adjustment = 1.0 - penalty
    #         else:
    #             if answer_count_difference < 0:
    ...
    #                 if answer_score > 0.3:  # Lower threshold for high quality
    #                     # Less severe penalty for high quality answers
    #                     penalty = min(missing_ratio * 0.3, 0.15)
    #                 else:
    #                     # Standard penalty for average or lower quality answers
    #                     penalty = min(missing_ratio * 0.5, 0.35)
    ...
    #                 if extra_ratio > 1.0:  # Significantly more answers than expected
    #                     penalty = min(extra_ratio * 0.2, 0.3)
    #                 else:
    ...
    #     # Debug logging
    #     logging.debug(f"Model {model}, Example: Condition {condition_score:.3f}->{balanced_condition_score:.3f}, " +
    #                 f"Answer {answer_score:.3f}->{balanced_answer_score:.3f}, " +
    #                 f"Citation {citation_score:.3f}->{balanced_citation_score:.3f}, " +
    #                 f"Cnt Diff {answer_count_difference}, Cite Cnt {citation_count:.2f}")
    ...
    #     # Only select models that have data in all experiments
    #     common_models = [model for model in all_models 
    #                     if all(model in balanced_metrics[exp] for exp in balanced_metrics)]
        
    #     if not common_models:
    #         logging.warning("No models found with data in all experiments")
    #         # Use models from main experiment instead
    #         common_models = list(balanced_metrics["main"].keys())
    ...
    #     # Experiment colors with enhanced styles
    #     experiment_colors = {
    #         "ground_truth": "#77c2a2",  # Green
    #         "main": "#f79a76",          # Orange
    #         "compare": "#a4b6de"        # Blue
    #     }
    ...
    #     # --- Plot answer scores comparison ---
    #     answer_scores = {
    #         "ground_truth": [balanced_metrics["ground_truth"].get(model, {}).get("balanced_answer_score", 0) for model in sorted_models],
    #         "main": [balanced_metrics["main"].get(model, {}).get("balanced_answer_score", 0) for model in sorted_models],
    #         "compare": [balanced_metrics["compare"].get(model, {}).get("balanced_answer_score", 0) for model in sorted_models]
    #     }
        
    #     # Add hatching patterns for better visual distinction
    #     gt_bars = ax1.bar(x - width, answer_scores["ground_truth"], width, 
    #                     label=experiment_labels["ground_truth"], 
    #                     color=experiment_colors["ground_truth"],
    #                     edgecolor='black',
    #                     hatch='////')
                        
    #     main_bars = ax1.bar(x, answer_scores["main"], width, 
    #                     label=experiment_labels["main"], 
    #                     color=experiment_colors["main"],
    #                     edgecolor='black',
    #                     hatch='\\\\\\')
                        
    #     comp_bars = ax1.bar(x + width, answer_scores["compare"], width, 
    #                     label=experiment_labels["compare"], 
    #                     color=experiment_colors["compare"],
    #                     edgecolor='black',
    #                     hatch='....')
    ...
    #     # Add labels
    #     for i, score in enumerate(answer_scores["ground_truth"]):
    #         ax1.text(i - width, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
    #     for i, score in enumerate(answer_scores["main"]):
    #         ax1.text(i, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
    #     for i, score in enumerate(answer_scores["compare"]):
    #         ax1.text(i + width, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
        
    #     ax1.set_title('Answer Score Comparison')
    #     ax1.set_ylabel('Average Answer Score')
    #     ax1.set_ylim(0, 1)
    #     ax1.set_xticks(x)
    #     ax1.set_xticklabels(sorted_models, rotation=0, ha='center')
        
    #     # --- Plot citation scores comparison ---
    #     citation_scores = {
    #         "ground_truth": [balanced_metrics["ground_truth"].get(model, {}).get("balanced_citation_score", 0) for model in sorted_models],
    #         "main": [balanced_metrics["main"].get(model, {}).get("balanced_citation_score", 0) for model in sorted_models],
    #         "compare": [balanced_metrics["compare"].get(model, {}).get("balanced_citation_score", 0) for model in sorted_models]
    #     }
        
    #     ax2.bar(x - width, citation_scores["ground_truth"], width, 
    #           label=experiment_labels["ground_truth"], 
    #           color=experiment_colors["ground_truth"],
    #           edgecolor='black',
    #           hatch='////')
              
    #     ax2.bar(x, citation_scores["main"], width, 
    #           label=experiment_labels["main"], 
    #           color=experiment_colors["main"],
    #           edgecolor='black',
    #           hatch='\\\\\\')
              
    #     ax2.bar(x + width, citation_scores["compare"], width, 
    #           label=experiment_labels["compare"], 
    #           color=experiment_colors["compare"],
    #           edgecolor='black',
    #           hatch='....')
    ...
    #     # Add labels
    #     for i, score in enumerate(citation_scores["ground_truth"]):
    #         ax2.text(i - width, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
    #     for i, score in enumerate(citation_scores["main"]):
    #         ax2.text(i, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
    #     for i, score in enumerate(citation_scores["compare"]):
    #         ax2.text(i + width, score + 0.01, f'{score:.2f}', ha='center', va='bottom', fontsize=8)
        
    #     ax2.set_title('Citation Score Comparison')
    #     ax2.set_ylabel('Average Citation Score')
    #     ax2.set_ylim(0, 1)
    #     ax2.set_xticks(x)
    #     ax2.set_xticklabels(sorted_models, rotation=0, ha='center')
    ...
    #     # Add title
    #     fig.suptitle('Model Performance Across Different Experimental Conditions', fontsize=16)
        
    #     # Add explanation caption
    #     caption = "Figure: Model performance in Answer Score and Citation Score, comparing answering without conditions,\n"
    #     caption += "answering based on identified conditions (Main Experiment), and answering based on ground truth conditions."
    ...
    #     # Save figures
    #     plt.savefig(self.output_dir / 'experiment_comparison.png', dpi=300, bbox_inches='tight')
    #     plt.savefig(self.output_dir / 'experiment_comparison.pdf', bbox_inches='tight')
        
    #     logging.info("Saved experiment comparison chart")
    #     return fig
