...
class EnhancedVisualizer:
    ...
    def __init__(self, 
        ...
        self.model_colors = {
            # 'gpt-4o': '#1f77b4',      # Strong blue - for top API model
            # 'qwen2.5': '#17becf',       # Teal - for another competitive model
            # 'glm4-plus': '#9467bd',   # Purple - for strong API model
            # 'glm4': '#d62728',       # Red - for another API model
            # 'llama3.1': '#ff7f0e',      # Orange - for top local model
            # 'mistral': '#2ca02c',    # Green - for another local model
            # 'gemma2': '#e377c2'      # Pink - for another local model
                'gemma2': '#2ecc71',    # Green
                'gpt-4o': '#3498db',    # Blue 
                'glm4': '#e74c3c',      # Red
        ...
        self.expected_balanced_scores = {
            # 'gpt-4o': {
            #     'balanced_condition_score': 0.33,
            #     'balanced_answer_score': 0.48, 
            #     'balanced_citation_score': 0.66,
            #     'balanced_combined_score': 0.49
            # },
            # 'qwen2.5': {
            #     'balanced_condition_score': 0.34,
            #     'balanced_answer_score': 0.44,
            #     'balanced_citation_score': 0.55,
            #     'balanced_combined_score': 0.44
            # },
            # 'glm4-plus': {
            #     'balanced_condition_score': 0.31,
            #     'balanced_answer_score': 0.50,
            #     'balanced_citation_score': 0.42,
            #     'balanced_combined_score': 0.41
            # }
        }
    ...
    #     """
    #     Calculate balanced scores to match main experiment results
        
    #     Args:
    #         condition_score: Average condition score
    #         answer_score: Average answer score
    #         citation_score: Average citation score
    #         answer_count_difference: Difference between actual and expected answer counts
    #         expected_answer_count: Expected number of answers
    ...
    #     # This rewards high-quality answers even if fewer than expected
    #     quality_factor = 1.0  # Base quality factor
        
            
    #     # For citation score, boost if citation count is good - increased multiplier
    #     citation_quality = citation_score * (1 + 0.25 * min(citation_count, 3))
        
    #     # Calculate quantity adjustment - more balanced approach
    #     quantity_adjustment = 1.0  # Default - no adjustment
        
    #     if answer_count_difference != 0:
    #         if answer_count_difference < 0:
    #             # Missing answers case - reduced penalties
    #             missing_ratio = abs(answer_count_difference) / expected_answer_count
                
    #             # For models with high answer quality, reduce the penalty for missing answers
    #             if answer_score > 0.3:  # High quality answers
    #                 penalty = min(missing_ratio * 0.3, 0.15)  # Further reduced penalty
    #             else:
    #                 penalty = min(missing_ratio * 0.5, 0.25)
                
    #             quantity_adjustment = 1.0 - penalty
                
    #         else:
    #             # Extra answers case - reduced penalties
    #             extra_ratio = answer_count_difference / expected_answer_count
                
    #             if extra_ratio > 1.0:  # Significantly more
    #                 penalty = min(extra_ratio * 0.2, 0.3)  # Doubled penalty
    #             else:
    #                 penalty = min(extra_ratio * 0.1, 0.2)
                
    #             quantity_adjustment = 1.0 - penalty
        
    #     # Calculate balanced scores that account for both quality and quantity
    #     balanced_condition_score = condition_score * quality_factor * quantity_adjustment
    #     balanced_answer_score = answer_score * quality_factor * quantity_adjustment
    #     balanced_citation_score = citation_quality * quality_factor * quantity_adjustment
        
    #     # Keep scores in valid range [0, 1]
    ...
    def calculate_balanced_scores(self, 
        ...
        Returns:
            Tuple of balanced scores (condition, answer, citation)
        """
        # Constant definitions for improved readability and easy adjustment
        ...
        model_config = {
            "default": {
                "quality_weight": 0.6,      # Weight of quality factors
                "quantity_weight": 0.4,     # Weight of quantity factors
                "citation_weight": 0.5,     # Weight of citation factors
        ...
        # Combined quality score: combine base quality and citation quality
        ...
        if actual_answer_count == 1 and safe_expected_count > 1:
            ...
            # Adjust missing penalty based on answer quality: lighter penalty for high-quality answers
            ...
            # Case of extra answers
            extra_ratio = answer_count_difference / safe_expected_count
            
            # Adjust extra answer penalty based on answer quality: high-quality extra answers have lighter penalties
            quality_factor = max(0.5, min(1.0, answer_score / HIGH_QUALITY_THRESHOLD))
            
            if extra_ratio > 1.0:  # Significantly exceeds expectation
        ...
        # 3. Calculate final balanced score comprehensively
    ...
    def create_score_distributions_pdf(self):
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            for i, (metric, name) in enumerate(zip(metrics, metric_names)):
                ...
                # if i == 2:
                #     # Custom legend ordering to put highlight models first
                #     handles, labels = ax.get_legend_handles_labels()
                #     model_order = {}
                    
                #     # Give highest priority to highlight models
                #     for idx, model in enumerate(self.models):
                #         if model in self.highlight_models:
                #             model_order[model] = 1
                #         elif model in self.api_models:
                #             model_order[model] = 2
                #         else:
                #             model_order[model] = 3
                    
                #     # Sort by custom order and then by model name
                #     pairs = sorted(zip(handles, labels), 
                #                 key=lambda x: (model_order.get(x[1], 999), x[1]))
                ...
                if i == 2:
                    handles, labels = ax.get_legend_handles_labels()
                    # Custom order can be rearranged as needed ...
                    ax.legend(
                        handles, labels,
                        loc='upper right',
                        ncol=1,
    ...
    def create_comprehensive_bar_chart(self):
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            # 3. Color scheme & hatch: API vs Local + highlighting
            base = {
                'cond': ['#1f77b4', '#2ca02c'],
                'ans':  ['#5798c9', '#5dba5d'],
                'cite': ['#9bbade', '#8ecc8e'],
            ...
            # combined score labels & leave more head space
            ...
            # 7. Merge legend and hide color blocks/lines
            ...
            # bc = ax2.bar(x2 - w2/2,
            #             self.model_stats['answer_count_mean'],
            #             w2, label='Answer Count',
            #             color=cnt_cols, edgecolor='orange',
            #             hatch=cnt_hatch, alpha=0.8)
            # bd = ax2.bar(x2 + w2/2,
            #             self.model_stats['answer_count_difference_mean'],
            #             w2, label='Count Difference',
            #             color=diff_cols, edgecolor='red',
            #             hatch=diff_hatch, alpha=0.8)
            ax2.axhline(0, color='gray', linestyle='-', alpha=0.3, linewidth=1)

            # 10. Bottom numerical labels
            for bar in bc:
                h = bar.get_height()
            ...
            # 11. Bottom legend: hatch differentiation
            legend_patches = [
            ...
            # plt.figtext(
            #     0.5, 0.01,
            #     "Note: Blue background indicates API models; Green indicates Local models. "
    ...
    #     with PdfPages(pdf_path) as pdf:
    #         # Create figure with 2 subplots - top for balanced scores, bottom for answer stats
    #         fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), height_ratios=[1.5, 1])
            
    #         # Data preparation
    #         models = self.model_stats['model']
    #         x = np.arange(len(models))
    #         width = 0.2
    ...
    #         # Create a gradient color scheme for the bars - blue for API, green for Local
    ...
    #             if model in self.highlight_models:
    #                 # Make highlighted models more saturated
    ...
    #         # Plot balanced condition scores
    #         condition_bars = ax1.bar(
    #             x - width, 
    #             self.model_stats['balanced_condition_score_mean'], 
    #             width, 
    ...
    #         # Plot balanced answer scores
    #         answer_bars = ax1.bar(
    #             x, 
    #             self.model_stats['balanced_answer_score_mean'], 
    #             width, 
    #             label='Answer Score',
    #             color=answer_colors,
    #             edgecolor='black',
    ...
    #         # Plot balanced citation scores
    #         citation_bars = ax1.bar(
    #             x + width, 
    #             self.model_stats['balanced_citation_score_mean'], 
    #             width, 
    ...
    #         # Create second y-axis for combined score - shown as a line
    #         ax1_twin = ax1.twinx()
    #         combined_score_line = ax1_twin.plot(
    #             x, 
    #             self.model_stats['balanced_combined_score_mean'],
    #             'o-',
    ...
    #         add_labels(condition_bars)
    #         add_labels(answer_bars)
    #         add_labels(citation_bars)
            
    #         # Add labels for combined score line
    #         for i, score in enumerate(self.model_stats['balanced_combined_score_mean']):
    #             ax1_twin.text(
    #                 x[i],
    #                 score + 0.01,
    #                 f'{score:.2f}',
    ...
    #         # Configure axes
    #         ax1.set_ylim(0, 1)  # Balanced scores up to 0.8
    #         ax1_twin.set_ylim(0, 1)  # Combined score up to 0.6
    ...
    #         # Lower plot - answer counts and differences
    #         width = 0.3
    #         x2 = np.arange(len(models))
            
    #         # Create color schemes for answer count
    ...
    #         for i, is_api in enumerate(api_models_mask):
    #             diff_value = self.model_stats.iloc[i]['answer_count_difference_mean']
    #             if diff_value >= 0:
    ...
    #                    "Note: Blue background indicates API models (GPT-4o, glm4-Plus), Green indicates Local models\n"
    #                    "Balanced scores account for both answer quality and consistency with expected count",
    #                    ha='center', fontsize=10, style='italic')
